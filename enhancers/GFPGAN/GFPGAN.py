...
class GFPGAN:
    ...
    def preprocess(self, imgs):
        # GFPGAN có vấn đề với batch size > 1, nên xử lý từng ảnh một cách an toàn
        # imgs: list hoặc numpy array of images (N, H, W, C)
    ...
    def enhance_batch(self, imgs):
        # Note: GFPGAN có thể gặp vấn đề với batch > 1, nên xử lý từng ảnh một
        if isinstance(imgs, list) and len(imgs) > 1:
            ...
            # imgs: list hoặc numpy array (N, H, W, C)
    ...
    def enhance(self, img):
        ...
        if isinstance(img, np.ndarray) and len(img.shape) == 3:  # Single image
