...
# Import torch trước khi import insightface/onnxruntime để preload CUDA DLLs[13]
import torch
...
def parse_args():
    ...
    parser.add_argument('--video_path', type=str, required=True, help='Filepath of video to enhance')
    parser.add_argument('--enhancer', type=str, choices=['gpen', 'gfpgan', 'codeformer', 'restoreformer'], required=True)
    parser.add_argument('--output_path', type=str, default=None, help='Filepath to save the enhanced video')
...
def enhance_video(video_path, enhancer_name, output_path=None):
    ...
    face_mask = cv2.cvtColor(face_mask, cv2.COLOR_GRAY2RGB) / 255
    batch_size = 1
    frame_buffer = []
    detector = RetinaFace("utils/scrfd_2.5g_bnkps.onnx",
    ...
    try:
        subprocess.call(['ffmpeg', '-i', video_path, '-vn', '-acodec', 'aac', '-b:a', '192k', audio_path])
    except Exception as e:
        print(f"Error extracting audio: {e}")
        raise
    try:
        subprocess.call(['ffmpeg', '-y', '-i', temp_video_path, '-i', audio_path, '-c:v', 'libx264', '-crf', '23', '-preset', 'medium', '-c:a', 'aac', '-b:a', '192k', '-movflags', '+faststart', output_path])
    except Exception as e:
